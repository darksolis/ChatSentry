...
function CS:EvaluateMessage(event, msg, sender, ...)
    ...
    -- Channel controls only decide where filtering runs. They never mute a channel.
    if ChatSentryDB.channels[event] == false then return nil end
    if SenderIsExempt(sender) then return nil end

    local blockedUser = MatchUser(sender)
    if blockedUser then return "user", blockedUser end

    local clean = CleanMessage(msg)
    for _, entry in ipairs(ChatSentryDB.words) do
        if MatchWord(clean, entry) then return "word", entry.value end
    end
    ...
    local reasonType, reason
    local categoryType, categoryReason = MatchSmartCategory(event, clean, channel)
    if categoryType then reasonType, reason = categoryType, categoryReason
    elseif IsRepeatMessage(event, sender, clean, channel) then reasonType, reason = "spam", "Repeated message"
    elseif IsBurstMessage(event, sender, channel) then reasonType, reason = "spam", "Message burst" end
...
local function CreateUI()
    ...
    do
        ...
        Backdrop(note, {0.07,0.085,0.11,1}, {0.26,0.34,0.46,1})
        MakeText(note, "Repeat and burst protection only run in public chat. Friends, guildmates, party and raid members still follow your exception settings.", 10, "LEFT", 12, 0, {0.78,0.83,0.90,1})
        p.checks = {}
        local smart = {
